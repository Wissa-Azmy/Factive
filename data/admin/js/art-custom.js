/**
 *	Art3araby Main JavaScript File
 *
 *	Theme by: www.art3araby.com
 **/
...
function replaceCheckboxes()
{
	...
	$(".checkbox-replace:not(.art-cb-replacement), .radio-replace:not(.art-cb-replacement)").each(function(i, el)
	{
		var $this = $(el),
		...
		$this.addClass('art-cb-replacement');
		...
		$input.on('change', function(ev)
		{
			if(is_radio)
			{
				//$(".art-cb-replacement input[type=radio][name='"+name+"']").closest('.art-cb-replacement').removeClass(checked_class);
				$(".art-cb-replacement input[type=radio][name='"+name+"']:not(:checked)").closest('.art-cb-replacement').removeClass(checked_class);
			}
...
function show_loading_bar(options)
{
	...
	var $ = jQuery,
		$loading_bar = $(".art-loading-bar");

	if($loading_bar.length == 0)
	{
		$loading_bar = $('<div class="art-loading-bar progress-is-hidden"><span data-pct="0"></span></div>');
