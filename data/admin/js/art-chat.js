...
;(function($, window, undefined)
{
	...
	$.extend(artChat, {
		...
		{
			// Conversation Close
			$conversation_window.on('click', '.conversation-close', artChat.close);
			...
			artChat.refreshUserIds();
			artChat.orderGroups();
			artChat.prefetchMessages();
			artChat.countUnreadMessages();
			artChat.puffUnreads();
			...
			$("body").on('click', '[data-toggle="chat"]', function(ev)
			{
				...
				artChat.toggleChat(with_animation, collapse_sidebar);
			});
		},
		...
		refreshUserIds: function()
		{
			$('#chat .chat-group a').each(function(i, el)
			{
				...
				if(typeof artChat.chat_history[id] == 'undefined')
				{
					var status = $this.data('status');

					if( ! status)
					{
						for(var i in artChat.statuses)
						{
							if($status.hasClass(artChat.statuses[i].class))
							{
								status = i;
								break;
							}
		...
		getStatus: function(id)
		{
			...
			var user = artChat.chat_history[id];
		...
		setStatus: function(id, status)
		{
			...
			if($user.length && this.statuses[status].class)
			{
				...
				artChat.chat_history[ $user.attr('id') ].status = status;
				this.orderGroups();
			}
		},
			...
			$chat.find('.chat-group a').each(function(i, el)
			{
				...
				if(history_container && history_container.length)
				{
					$($(history_container)).find('> li').each(function(j, el2)
					{
						...
						artChat.pushMessage(id, message, from, time, fromOpponent, unread)
					});
				}
		...
		countUnreadMessages: function(id)
		{
			...
			if( ! id)
			{
				for(var id in artChat.chat_history)
				{
					var user = artChat.chat_history[id],
		...
		puffUnreads: function()
		{
			...
			{
				var entry = artChat.chat_history[i],
