...
class twitter {
	...
	public function fetch_followers($limit) {
		...
		$followershtml = '';
		$ch = curl_init();
		$target = 'http://api.twitter.com/1/statuses/followers/'.$this->user.'.xml';
		curl_setopt($ch, CURLOPT_URL, $target);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
		...
		foreach ($followers->user as $follower) { 
			if ($counter >= $limit) { break; }
  			$followershtml .= '<a target="_top" href="http://www.twitter.com/'.$follower->screen_name.'"><img border="0" width=24 height=24 src="'. str_replace('normal','mini',$follower->profile_image_url) .'" alt="'. $follower->screen_name .'" title="'. $follower->screen_name .'"></a>';
			++$counter;
		}
